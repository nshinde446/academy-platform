...
export const outcomeKeys = {
  ...
  range: (branchId: string, from: string, to: string) =>
    [...outcomeKeys.all, branchId, from, to] as const,
};
...
export function useOutcomeInsights(
  ...
  return useQuery<OutcomeResponse>({
    queryKey: outcomeKeys.range(branchId ?? "", fromIsoDate, toIsoDate),
    queryFn: async () => {
      ...
      const res = await apiClient.get<OutcomeResponse>(
        "/api/v1/lectures/insights/outcomes",
        { params },
      );
      return res.data;
    },
